...
import { Search, MapPin, Sparkles, Zap, ShieldCheck, MessageSquare, ArrowRight, Building2, Briefcase, PlusCircle } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 space-y-8">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-emerald-400 font-extrabold text-xs uppercase tracking-wider bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                  Hiring Employers & Recruiters
                </span>
                <span className="text-amber-400 text-xs font-bold animate-pulse">● Instant Approval Portal</span>
              </div>
              <h2 className="text-base sm:text-lg font-black text-white mt-0.5">
                Want to Hire? Register now to post a job instantly!
              </h2>
              <p className="text-xs text-slate-300">
                Post your job today and find top verified remote & hybrid talent in Pakistan and worldwide.
              </p>
            </div>
          </div>
        ...
        <div className="text-center max-w-4xl mx-auto">
          ...
          <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-slate-900/90 border border-emerald-500/30 text-emerald-400 text-xs font-semibold mb-6 shadow-xl backdrop-blur-md">
            ...
            <span>#1 Unified Remote Portal & Automated CV Engine for Pakistan & Global</span>
          </div>

          {/* Main Heading */}
          <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight leading-tight">
            Find Your Next <span className="bg-gradient-to-r from-emerald-400 via-teal-300 to-indigo-400 bg-clip-text text-transparent">Hybrid or Remote</span> Role
          </h1>

          {/* Subheading */}
          <p className="mt-4 text-base sm:text-xl text-slate-300 leading-relaxed font-normal max-w-2xl mx-auto">
            Browse verified opportunities globally or target localized jobs across <span className="text-emerald-400 font-semibold">Pakistan</span> down to Province, City, and District levels. Build ATS-optimized resumes in minutes.
          </p>
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              ...
              className="w-full sm:w-auto px-8 py-4 rounded-xl bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600 text-slate-950 font-bold text-base shadow-xl shadow-emerald-500/20 hover:shadow-emerald-500/35 hover:scale-[1.02] active:scale-95 transition-all duration-200 flex items-center justify-center space-x-2"
            >
              <Search className="w-5 h-5 text-slate-950" />
              <span>Browse {totalJobsCount} Verified Jobs</span>
              <ArrowRight className="w-4 h-4 ml-1 text-slate-950" />
            </button>

            <button
              onClick={onCvClick}
              className="w-full sm:w-auto px-8 py-4 rounded-xl bg-slate-900 hover:bg-slate-800 text-white border border-slate-700/80 font-semibold text-base shadow-lg hover:border-indigo-500/50 hover:scale-[1.02] active:scale-95 transition-all duration-200 flex items-center justify-center space-x-2"
            >
          ...
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-3 text-left">
            <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-sm">
              <div className="flex items-center space-x-2 text-emerald-400 font-bold text-sm">
                ...
                <span>Localized PK Sorting</span>
              </div>
