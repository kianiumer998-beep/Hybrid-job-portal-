        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6 text-sm text-slate-300">
          ...
          {(job.clippingImageUrl || job.mediaUrl) && (
            <div className="p-4 bg-slate-950/90 border border-teal-500/30 rounded-xl space-y-3">
              <div className="flex items-center justify-between text-xs font-bold text-teal-300">
                ...
                {job.newspaperName && <span>{job.newspaperName} ({job.newspaperDate || 'Recent Edition'})</span>}
              </div>
              <div className="rounded-lg overflow-hidden border border-slate-800 bg-black max-h-72 flex justify-center">
                <img
                  src={job.clippingImageUrl || job.mediaUrl}
                  alt="Official Vacancy Advertisement Clipping"
                  className="object-contain max-h-72 w-full hover:scale-102 transition-transform duration-300"
          ...
          {job.extractedText && (
            <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-xl space-y-2">
              <h4 className="text-xs uppercase font-bold text-slate-400 tracking-wider">Extracted Gazette / Document Text</h4>
              <div className="p-3 bg-slate-900 rounded-lg text-xs font-mono text-slate-300 max-h-40 overflow-y-auto whitespace-pre-wrap leading-relaxed border border-slate-800">
                {job.extractedText}
              </div>
            </div>
          )}
