...
export interface ScraperSourceItem {
  ...
  lastRun?: string;
  scrapedCount?: number;
  healthStatus?: 'Healthy' | 'Jobs Found' | 'No Jobs' | '404' | '403' | 'Timeout' | 'Invalid PDF' | 'HTML' | 'Fetch Error' | 'Database Error' | 'Disabled' | 'healthy' | 'warning' | 'error' | string;
  ...
  const filteredSources = useMemo(() => {
    return sourcesList.filter(source => {
      ...
      if (statusFilter !== 'all') {
        ...
        if (statusFilter === 'Error' && source.healthStatus !== 'error' && !['404', '403', 'Timeout', 'Invalid PDF', 'HTML', 'Fetch Error', 'Database Error'].includes(source.healthStatus || '')) return false;
      }
      ...
      if (healthFilter !== 'all') {
        ...
        if (healthFilter === 'Failed') {
          if (!['404', '403', 'Timeout', 'Invalid PDF', 'HTML', 'Fetch Error', 'Database Error', 'error'].includes(actualHealth)) return false;
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center space-x-2 flex-wrap gap-2">
              ...
              {sourcesList.some(s => ['404', '403', 'Timeout', 'Invalid PDF', 'HTML', 'Fetch Error', 'Database Error'].includes(s.healthStatus || '')) && (
                <button
                  type="button"
                ...
                >
                  ...
                  <span>Retry All Failed ({sourcesList.filter(s => ['404', '403', 'Timeout', 'Invalid PDF', 'HTML', 'Fetch Error', 'Database Error'].includes(s.healthStatus || '')).length})</span>
                </button>
              )}
            </div>
          </div>
                            ...
                            <div className="flex flex-col space-y-1">
                              <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold w-fit ${
                                source.healthStatus === 'Healthy' || source.healthStatus === 'Jobs Found'
                                  ...
                                  ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30'
                                  : source.healthStatus === 'Database Error'
                                  ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30'
                                  : source.healthStatus === 'Fetch Error'
                                  ? 'bg-rose-500/20 text-rose-300 border border-rose-500/30'
              ...
              <div className="space-y-3 pt-2">
                <div className="text-[11px] text-slate-400 p-2 bg-slate-950 rounded-lg border border-slate-800">
                  ...
                  <span className="font-bold text-rose-300">
                    {sourcesList.filter(s => ['404', '403', 'Timeout', 'Invalid PDF', 'HTML', 'Fetch Error', 'Database Error'].includes(s.healthStatus || '')).length} sources
                  </span>
                </div>
